 ...
 *
 * DEPRECATED: Cloud Function used with a Cloud Storage trigger that activates upon file upload finish
 */
...
async function getSignedUploadURL(
    ...
    const signedURLResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/image/signed`, {
        ...
        body: JSON.stringify({ objectName: fileName, operatingSystem: operatingSystem }),
    });
...
const AddImageModal = ({ open, onClose }: ModalProps) => {
    ...
    const operatingSystems = ["debian-11", "debian-10", "debian-9",
        "ubuntu-2204", "ubuntu-2004", "ubuntu-1804", "windows-2022",
        "windows-2019", "windows-11-x64-byol", "windows-10-x64-byol",
        "windows-8-x86-byol", "windows-8-x64-byol"];
                        ...
                        <div className="space-y-4">
                            ...
                            <form id="createVirtualMachineForm" onSubmit={handleFormSubmit}>
                                <div className="grid gap-y-4">
                                    {/* Form Group */}
                                    <div>
                                        <label htmlFor="imageName" className="block text-sm mb-2 dark:text-white">Operating System</label>
                                        <Listbox value={operatingSystem} onChange={setOperatingSystem}>
                                            <div className='relative'>
                                                ...
                                                <Listbox.Button className="relative cursor-default text-left py-3 px-4 block w-full border rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400">
                                                    {operatingSystem || "Select an Image"}
                                                ...
                                                <Listbox.Options className="border-gray-200 absolute mt-1 max-h-40 w-full overflow-auto rounded-md py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm dark:bg-gray-800 dark:border-gray-700">
                                                    {operatingSystems?.map((codename) => (
                                                        <Listbox.Option
                                                            key={codename}
                                                            value={codename}
                                                            className={({ active }) =>
